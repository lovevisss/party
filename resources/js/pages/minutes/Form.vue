...
import { Archive, BookmarkPlus, Download, FileCheck2, FileUp, ListPlus, Save, Search, Trash2, X } from 'lucide-vue-next';
import BusinessLayout from '@/layouts/BusinessLayout.vue';

const props = defineProps<{ minute: any | null; organizations: any[]; participantPresets: any[] }>();
...
const availablePresets = computed(() => props.participantPresets.filter((preset) => Number(preset.organization_id) === Number(form.organization_id)));
...
const applyPreset = () => {
    const preset = availablePresets.value.find((item) => item.id === Number(selectedPresetId.value));
    if (!preset) return;
    if (form.participants.length && !confirm('套用常用名单将覆盖当前已选择的人员，是否继续？')) return;
    const availableItems = preset.items.filter((item: any) => item.is_external || item.person?.status === 'active');
    ...
    presetMessage.value = skipped ? `已套用“${preset.name}”，其中 ${skipped} 名停用人员被跳过。` : `已套用“${preset.name}”，共 ${availableItems.length} 人。`;
};
const savePreset = () => {
    const name = presetName.value.trim();
    if (name.length < 2) { presetMessage.value = '请输入至少 2 个字的清单名称。'; return; }
    if (!form.participants.length) { presetMessage.value = '当前人员清单为空，无法保存。'; return; }
    router.post('/participant-presets', { organization_id: form.organization_id, name, participants: form.participants }, {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => { presetMessage.value = `常用名单“${name}”已保存；同名清单会自动覆盖更新。`; presetName.value = ''; },
    });
};
const deletePreset = () => {
    const preset = availablePresets.value.find((item) => item.id === Number(selectedPresetId.value));
    if (!preset || !confirm(`确认删除常用名单“${preset.name}”吗？`)) return;
    router.delete(`/participant-presets/${preset.id}`, { preserveScroll: true, onSuccess: () => { selectedPresetId.value = ''; presetMessage.value = '常用名单已删除。'; } });
};
...
<template>
  <BusinessLayout :title="minute ? '编辑会议纪要' : '新建会议纪要'" eyebrow="党总支会议 · 结构化归档">
    <form class="space-y-5" @submit.prevent="save">
      <section class="form-card"><div class="section-head"><span>01</span><div><h2>基本信息</h2><p>会议编号与召开时间</p></div></div><div class="form-grid"><label class="field"><span>所属学院</span><select v-model="form.organization_id" required><option v-for="organization in organizations" :key="organization.id" :value="organization.id">{{ organization.name }}</option></select></label><label class="field"><span>会议年度</span><input v-model="form.meeting_year" type="number"></label><label class="field"><span>会议序号</span><input v-model="form.sequence_no" type="number" min="1" max="999"></label><label class="field md:col-span-3"><span>会议名称</span><input v-model="form.title" maxlength="200" placeholder="例如：XX学院2026年第1次党总支会议"></label><label class="field"><span>开始时间</span><input v-model="form.meeting_start_at" type="datetime-local"></label><label class="field"><span>结束时间</span><input v-model="form.meeting_end_at" type="datetime-local"></label></div></section>
      <section class="form-card"><div class="section-head"><span>02</span><div><h2>人员情况</h2><p>从在职人员库精确选择，或套用个人常用名单</p></div></div><div class="mb-5 grid gap-3 border border-[#d8e2dd] bg-[#f6faf8] p-4 lg:grid-cols-2"><div><p class="mb-2 flex items-center gap-2 text-sm font-medium text-[#245446]"><ListPlus :size="17" />快速套用常用名单</p><div class="flex gap-2"><select v-model="selectedPresetId" class="control flex-1"><option value="">选择已保存的名单</option><option v-for="preset in availablePresets" :key="preset.id" :value="preset.id">{{ preset.name }}（{{ preset.items.length }} 人）</option></select><button type="button" class="btn-secondary" :disabled="!selectedPresetId" @click="applyPreset">套用</button><button type="button" class="border border-[#d6c7c2] px-3 text-[#9f3f36] disabled:opacity-40" :disabled="!selectedPresetId" title="删除名单" @click="deletePreset"><Trash2 :size="16" /></button></div></div><div><p class="mb-2 flex items-center gap-2 text-sm font-medium text-[#245446]"><BookmarkPlus :size="17" />保存当前人员配置</p><div class="flex gap-2"><input v-model="presetName" class="control flex-1" maxlength="50" placeholder="例如：学院党总支会议固定参会人员" @keyup.enter.prevent="savePreset"><button type="button" class="btn-secondary" @click="savePreset">保存名单</button></div></div><p v-if="presetMessage" class="text-xs text-[#65736e] lg:col-span-2">{{ presetMessage }}</p></div><div class="flex gap-2"><input v-model="query" class="control flex-1" placeholder="输入姓名或工号" @keyup.enter.prevent="search"><button type="button" class="btn-secondary" @click="search"><Search :size="16" />搜索</button></div><div v-if="results.length" class="mt-3 max-h-56 overflow-y-auto border border-[#ded7c9]"><div v-for="person in results" :key="person.id" class="flex flex-wrap items-center gap-2 border-b px-3 py-2 text-sm"><span class="mr-auto font-medium">{{ person.name }} <small class="font-normal text-slate-500">{{ person.employee_no }} · {{ person.organization?.name }}</small></span><button v-for="(label, key) in roles" :key="key" type="button" class="rounded border px-2 py-1 text-xs" @click="add(person, key)">+ {{ label }}</button></div></div><div class="mt-4 flex flex-wrap gap-2"><span v-for="(person, index) in form.participants" :key="index" class="inline-flex items-center gap-2 rounded-full bg-[#edf3f0] px-3 py-1.5 text-xs text-[#245446]"><b>{{ roles[person.role_type] }}</b>{{ person.display_name }}<button type="button" @click="form.participants.splice(index, 1)"><X :size="13" /></button></span></div></section>
