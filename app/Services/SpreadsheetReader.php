...
class SpreadsheetReader
{
    ...
    public function rows(UploadedFile $file): array
    {
        if (strtolower($file->getClientOriginalExtension()) === 'csv') {
            return $this->csv($file->getRealPath());
        }
    ...
    private function csv(string $path): array
    {
        ...
        while (($r = fgetcsv($h)) !== false) {
            $rows[] = array_map(function ($value): string {
                $converted = mb_convert_encoding((string) $value, 'UTF-8', 'UTF-8,GB18030');
    ...
    private function xlsx(string $path): array
    {
        ...
        if ($zip->open($path) !== true) {
            throw new RuntimeException('无法读取 XLSX 文件。');
        }
        $shared = [];
        ...
        if ($raw) {
            ...
            if ($xml === false) {
                throw new RuntimeException('XLSX 共享字符串格式错误。');
            }
            foreach ($xml->si as $si) {
                $shared[] = trim((string) $si->t ?: implode('', array_map('strval', iterator_to_array($si->r->t ?? []))));
            }
        }
        $sheet = $zip->getFromName('xl/worksheets/sheet1.xml');
        $zip->close();
        if (! $sheet) {
            throw new RuntimeException('XLSX 缺少首个工作表。');
        }
        $xml = simplexml_load_string($sheet);
        if ($xml === false) {
            throw new RuntimeException('XLSX 工作表格式错误。');
        }
        $rows = [];
